...
func (c *HTTPClient) Aggregate(ctx context.Context, aggReq *types.AggregateRequest) error {
	...
	if resp.StatusCode != http.StatusOK {
		return fmt.Errorf("the service reponded with non 200 status code: %d", resp.StatusCode)
	}
	resp.Body.Close()
	return nil
}
...
func (c *HTTPClient) GetInvoice(ctx context.Context, id int) (*types.Invoice, error) {
	...
	logrus.Infof("sending request to %s", endpoint)
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("the service reponded with non 200 status code: %d", resp.StatusCode)
	}
